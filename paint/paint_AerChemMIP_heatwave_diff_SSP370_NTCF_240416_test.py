...
def plot_change_hw_attribute_multimodel(hist, ssp, sspntcf, left_string, figname, lon, lat, ct_level=np.linspace(-10., 10., 21)):
    '''
    This function is to plot the changes in the wet day among the SSP370 and SSP370lowNTCF
...
if __name__ == '__main__':
    ...
    plot_change_hw_attribute(np.nanmean(model_set['hist_hw_tasmin_multimodel'].data[:, 2, -20:], axis=0), np.nanmean(model_set['ssp_hw_tasmin_multimodel'].data[num0, 2, -20:], axis=0), np.nanmean(model_set['ntcf_hw_tasmin_multimodel'].data[num0, 2, -20:], axis=0), 'Heat Wave intensity (tasmin) {}'.format(models_label[num0]), 'Heat Wave intensity (tasmin) {}'.format(models_label[num0]), lon, lat, np.linspace(-150., 150., 16))
